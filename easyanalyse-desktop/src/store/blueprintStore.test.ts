...
function createAgentCandidate(document: DocumentFile = createDocument({ document: { id: 'candidate-doc', title: 'Candidate' } })): AgentBlueprintCandidate {
  return {
    title: 'Agent candidate',
    summary: 'Generated by agent.',
    rationale: 'Test candidate insertion.',
    tradeoffs: [],
    document,
    issues: [],
  }
...
describe('blueprintStore', () => {
  ...
  it('stores agent candidates after rebinding stale same-document workspace metadata', async () => {
    const savedDocument = createDocument({ document: { id: 'doc-agent-main', title: 'Saved' } })
    const dirtyDocument = createDocument({
      document: { id: 'doc-agent-main', title: 'Dirty title' },
      devices: [
        ...savedDocument.devices,
        { id: 'c1', name: 'C1', kind: 'capacitor', terminals: [] },
      ],
    ...
    useBlueprintStore.setState({
      workspace: createEmptyBlueprintWorkspace({
        mainDocument: {
          documentId: 'doc-agent-main',
          path: '/tmp/old.easyanalyse.json',
          hash: 'stale-hash',
          hashAlgorithm: 'easyanalyse-document-canonical-sha256-v1',
        },
      }),
    ...
    const inserted = await useBlueprintStore.getState().addAgentBlueprintCandidates(
      ...
      { mainDocument: dirtyDocument, filePath: '/tmp/current.easyanalyse.json' },
    )

    const state = useBlueprintStore.getState()
    expect(inserted).toHaveLength(1)
    expect(state.workspace?.blueprints).toHaveLength(1)
    expect(state.selectedBlueprintId).toBe(inserted[0]?.id)
    expect(state.workspace?.mainDocument).toMatchObject({
      documentId: 'doc-agent-main',
      path: '/tmp/current.easyanalyse.json',
      hash: dirtyHash,
    })
  ...
  it('refuses agent candidates for a definitely different loaded document', async () => {
    ...
    useBlueprintStore.setState({
      workspace: createEmptyBlueprintWorkspace({
        mainDocument: {
          ...
          path: '/tmp/other.easyanalyse.json',
          hash: 'other-hash',
          hashAlgorithm: 'easyanalyse-document-canonical-sha256-v1',
        },
      }),
    ...
    const inserted = await useBlueprintStore.getState().addAgentBlueprintCandidates(
      [createAgentCandidate()],
      { mainDocument: document, filePath: '/tmp/current.easyanalyse.json' },
    )

    expect(inserted).toEqual([])
    expect(useBlueprintStore.getState().workspace?.blueprints).toEqual([])
    expect(useBlueprintStore.getState().workspace?.mainDocument?.documentId).toBe('other-document')
    expect(useBlueprintStore.getState().dirty).toBe(false)
  })
  ...
  it('writes agent thread state into workspace extensions without modifying blueprint records', () => {
    const workspace = createEmptyBlueprintWorkspace({ workspaceId: 'workspace-agent-threads' })
    const agentThreads: AgentThreadWorkspace = {
    ...
    useBlueprintStore.getState().setWorkspaceAgentThreads(agentThreads)

    expect(useBlueprintStore.getState().workspace?.blueprints).toEqual([])
    expect(useBlueprintStore.getState().workspace?.extensions?.agentThreads).toBe(agentThreads)
    expect(useBlueprintStore.getState().dirty).toBe(true)
  })
})
