...
> 执行规则：cronjob 每 30 分钟启动一次；每轮先通过 `automation/autonomous_lock.py` / cron preflight 原子获取 `automation/.autonomous_run.lock` 防并发，且必须保存本轮 `runId`（preflight 为 `EASYANALYSE_PREFLIGHT_RUN_ID`，手动 acquire 为 stdout `runId=...`）。缺 runId 时 fail closed，不得施工；结束前必须 `release --run-id <本轮runId>`。然后优先完成一个最小任务。完成后把 `[ ]` 改为 `[x]`，必要时追加 commit/test 信息。阻塞任务用 `[!]` 标记并写明原因。
