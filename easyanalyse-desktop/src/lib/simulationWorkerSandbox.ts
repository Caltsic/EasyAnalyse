  ...
  return new Promise<SimulationWorkerRunResult>((resolve, reject) => {
    ...
    worker.onmessage = (event) => {
      ...
      if (message.type !== 'result') {
        fail({
          code: 'protocol-error',
          message: `Simulation worker returned an unsupported protocol message type: ${message.type}.`,
          detail: message,
        })
        return
      }
...
function formatWorkerErrorEvent(event: WorkerErrorEventLike): string {
  const message = typeof event.message === 'string' && event.message.length > 0
    ? event.message
    : 'Simulation worker script error.'
  ...
  const location = filename || line !== null
    ? ` (${[filename, line, column].filter((item) => item !== '' && item !== null).join(':')})`
    : ''
...
function disableUnsafeGlobals() {
  ...
  try {
    ...
    try { self.postMessage = blocked; } catch (_) {}
  }
}
...
function loadUserSimulationApi(script) {
  ...
  return factory(module, exports, createUserSelf());
}

function createUserSelf() {
  return Object.freeze({
    postMessage: function blockedPostMessage() {
      throw new Error('Simulation worker direct postMessage is disabled. Return a result from run/simulate instead.');
    },
  });
}
