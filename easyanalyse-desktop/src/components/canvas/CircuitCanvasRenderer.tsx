...
export interface CircuitCanvasRendererProps {
  ...
  onUpdateNetworkLine?: (id: string, patch: Partial<NetworkLineViewDefinition>) => void
  onSetSelection?: (selection: EditorSelection | null) => void
  onSetDeviceGroupSelection?: (ids: string[]) => void
  onPlacePendingDevice?: (position?: Point) => string | null
  onFocusDevice?: (deviceId: string | null, target?: { center: Point; zoom: number } | null) => void
  onFocusNetworkLine?: (networkLineId: string | null) => void
  onClearFocus?: () => void
...
export function CircuitCanvasRenderer({
  document,
  theme = 'light',
  locale = 'zh-CN',
  interactive = false,
  ...
  onMoveDevice = noop,
  onMoveDevices = noop,
  onRepositionTerminal = noop,
  onUpdateNetworkLine = noop,
  onSetSelection = noop,
  onSetDeviceGroupSelection = noop,
  onPlacePendingDevice = () => null,
  onFocusDevice = noop,
  onFocusNetworkLine = noop,
  onClearFocus = noop,
  onResetViewportToOrigin = noop,
}: CircuitCanvasRendererProps) {
  ...
  const handleStageClick = (event: { evt: MouseEvent; target: Konva.Node }) => {
    ...
    onSetSelection({ entityType: 'document' })
    onClearFocus()
  }
            ...
            {insights.networkLines.map((networkLine) => {
              ...
              return (
                <Group
                  ...
                  draggable={canEdit && focusProgress < 0.05 && !pendingDeviceShape}
                  onClick={(evt) => {
                    ...
                    evt.cancelBubble = true
                    onSetSelection({ entityType: 'networkLine', id: networkLine.id })
                  ...
                  onDblClick={(evt) => {
                    ...
                    evt.cancelBubble = true
                    onSetSelection({ entityType: 'networkLine', id: networkLine.id })
                    onFocusNetworkLine(networkLine.id)
                  }}
                  onDragStart={() => {
                    suppressClickRef.current = true
                  }}
                  onDragEnd={(evt) => {
                    ...
                    onUpdateNetworkLine(networkLine.id, {
                      position: {
              ...
              return (
                <Group
                  ...
                  onClick={(evt) => {
                    ...
                    evt.cancelBubble = true
                    onSetSelection({ entityType: 'device', id: device.id })
                  ...
                  onDragEnd={(evt) => {
                    ...
                    if (groupedSelection && selectedDeviceSet.size > 1) {
                      onMoveDevices([...selectedDeviceSet], {
                        x: nextPosition.x - device.bounds.x,
                        y: nextPosition.y - device.bounds.y,
                      })
                    } else {
                      onMoveDevice(device.id, nextPosition)
                    }
                  ...
                  {device.terminals.map((terminal) => {
                    ...
                    return (
                      <Group
                        ...
                        x={point.x}
                        y={point.y}
                        draggable={canEdit && !pendingDeviceShape && focusProgress < 0.05}
                        onClick={(evt) => {
                          if (pendingDeviceShape) {
                            return
                          }
                          evt.cancelBubble = true
                          onSetSelection({ entityType: 'terminal', id: terminal.id })
                        }}
                        onDragStart={(evt) => {
                          evt.cancelBubble = true
                          suppressClickRef.current = true
      ...
      {(footerMessage || focusedDeviceId || focusedLabelKey || focusedNetworkLineId) && (
        <div className="canvas-footer">
          ...
          <div className="canvas-footer__actions">
            {(focusedDeviceId || focusedLabelKey || focusedNetworkLineId) && (
              <button className="ghost-button" onClick={() => onClearFocus()}>
                {t('exitFocus')}
              </button>
            )}
          </div>
