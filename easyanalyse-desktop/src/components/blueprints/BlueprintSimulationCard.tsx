    ...
    <section className="blueprint-simulation-card" aria-label={t('simulationPreview')}>
      <div className="blueprint-simulation-card__header">
        ...
        <button className="ghost-button" type="button" onClick={running ? cancelPreview : () => void runPreview()}>
          {running ? <Square size={14} strokeWidth={2.1} aria-hidden="true" /> : <Play size={14} strokeWidth={2.1} aria-hidden="true" />}
      ...
      <div className={`blueprint-simulation-card__status is-${status}`}>
        ...
        {result ? (
          <span>
            {t('simulationPointCount', { count: result.points.length })} / {t('simulationDuration', { ms: Math.round(result.durationMs) })}
          </span>
        ) : null}
      </div>
...
function createRunInput(
  ...
  const defaultInput = isRecord(artifact.defaultInput) ? structuredClone(artifact.defaultInput) as SimulationWorkerInput : {}
