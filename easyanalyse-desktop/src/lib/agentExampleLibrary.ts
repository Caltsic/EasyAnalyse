    ...
    document: {
      ...
      view: {
        ...
        devices: {
          ...
          c1: { position: { x: 240, y: 96 }, shape: 'rectangle' },
          tp1: { position: { x: 400, y: 96 }, shape: 'circle' },
        },
        networkLines: {
          vin: { label: 'VIN', position: { x: 24, y: 128 } },
          vout: { label: 'VOUT', position: { x: 208, y: 128 } },
          gnd: { label: 'GND', position: { x: 240, y: 240 } },
        },
      },
    },
  },
    ...
    document: {
      ...
      view: {
        ...
        devices: {
          ...
          rf: { position: { x: 240, y: 64 }, shape: 'rectangle' },
          u1: { position: { x: 320, y: 144 }, shape: 'triangle' },
        },
        networkLines: {
          vin: { label: 'VIN', position: { x: 24, y: 192 } },
          n_inv: { label: 'N_INV', position: { x: 256, y: 192 } },
          vout: { label: 'VOUT', position: { x: 488, y: 192 } },
          gnd: { label: 'GND', position: { x: 320, y: 288 } },
          vcc: { label: 'VCC', position: { x: 320, y: 32 } },
        },
      },
    },
  },
    ...
    document: {
      ...
      view: {
        ...
        devices: {
          ...
          u485: { position: { x: 304, y: 128 }, shape: 'rectangle' },
          rt: { position: { x: 520, y: 80 }, shape: 'rectangle' },
          tvs: { position: { x: 520, y: 184 }, shape: 'rectangle' },
          j1: { position: { x: 704, y: 128 }, shape: 'rectangle' },
          cdec: { position: { x: 304, y: 304 }, shape: 'rectangle' },
        },
        networkLines: {
          mcu_tx: { label: 'MCU_TX', position: { x: 240, y: 144 } },
          mcu_rx: { label: 'MCU_RX', position: { x: 240, y: 176 } },
          rs485_de: { label: 'RS485_DE', position: { x: 240, y: 208 } },
          rs485_a: { label: 'RS485_A', position: { x: 488, y: 144 } },
          rs485_b: { label: 'RS485_B', position: { x: 488, y: 176 } },
          vcc_3v3: { label: 'VCC_3V3', position: { x: 304, y: 48 } },
          gnd: { label: 'GND', position: { x: 304, y: 400 } },
        },
      },
    },
  },
...
export function selectAgentReferenceExamples(prompt: string, currentDocument: DocumentFile | null, limit = 2): AgentReferenceExample[] {
  ...
  const scored = AGENT_REFERENCE_EXAMPLES.map((example, index) => ({
    example,
    index,
    score: example.tags.reduce((total, tag) => total + (text.includes(tag.toLowerCase()) ? 2 : 0), 0) + (example.id.includes('mcu') && /interface|rs485|can|uart|mcu|stm32|digital/.test(text) ? 3 : 0),
  }))
  return scored
